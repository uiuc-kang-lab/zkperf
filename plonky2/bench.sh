...
{ RUST_LOG=debug /usr/bin/time -v "$ECDSA_DIR"/target/release/standard; } 2> "$ECDSA_LOG"
...
touch $MNIST_LOG
"$ML_DIR"/target/release/time_circuit mnist "$ML_DIR"/examples/mnist/model.msgpack "$ML_DIR"/examples/mnist/inp.msgpack build "$MNIST_OUTPUT" 25
...
touch $DLRM_LOG
"$ML_DIR"/target/release/time_circuit dlrm "$ML_DIR"/examples/dlrm/model.msgpack "$ML_DIR"/examples/dlrm/inp.msgpack build "$DLRM_OUTPUT" 25
...
touch "$SCRIPT_DIR"/breakdowns.csv
