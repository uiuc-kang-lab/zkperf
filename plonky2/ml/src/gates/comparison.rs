...
impl<F: RichField + Extendable<D>, const D: usize> ComparisonGate<F, D> {
  ...
  pub fn chunk_bits() -> usize {
    // ceil_div_usize(self.num_bits, 64)
    2
  }

  pub fn wire_first_input() -> usize {
    0
  }

  pub fn wire_second_input() -> usize {
    1
  }

  pub fn wire_result() -> usize {
    2
  }

  pub fn wire_most_significant_diff() -> usize {
    3
  }

  pub fn wire_first_chunk_val(chunk: usize) -> usize {
    // debug_assert!(chunk < 16);
    4 + chunk
  }

  pub fn wire_second_chunk_val(chunk: usize) -> usize {
    // debug_assert!(chunk < 16);
    // 4 + 16 + chunk
    4 + 16 + chunk
  }

  pub fn wire_equality_dummy(chunk: usize) -> usize {
    // debug_assert!(chunk < 16);
    // 4 + 2 * 16 + chunk
    4 + 2 * 16 + chunk
  }

  pub fn wire_chunks_equal(chunk: usize) -> usize {
    // debug_assert!(chunk < 16);
    // 4 + 3 * 16 + chunk
    4 + 3 * 16 + chunk
  }

  pub fn wire_intermediate_value(chunk: usize) -> usize {
    // debug_assert!(chunk < 16);
    // 4 + 4 * 16 + chunk
    4 + 4 * 16 + chunk
  }
  ...
  pub fn wire_most_significant_diff_bit(bit_index: usize) -> usize {
    // 4 + 5 * 16 + bit_index
    4 + 5 * 16 + bit_index
  }
}
...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for ComparisonGate<F, D> {
  ...
  fn eval_unfiltered(&self, vars: EvaluationVars<F, D>) -> Vec<F::Extension> {
    ...
    let first_chunks: Vec<F::Extension> = (0..16)
      .map(|i| vars.local_wires[Self::wire_first_chunk_val(i)])
      .collect();
    let second_chunks: Vec<F::Extension> = (0..16)
    ...
    let result = vars.local_wires[Self::wire_result()];
    let le = most_significant_diff_bits[Self::chunk_bits()];
    constraints.push((F::Extension::ONE - le) * (result - second_input));
    constraints.push(le * (result - first_input));
  ...
  fn eval_unfiltered_circuit(
    ...
    // Get chunks and assert that they match
    let first_chunks: Vec<ExtensionTarget<D>> = (0..16)
      .map(|i| vars.local_wires[Self::wire_first_chunk_val(i)])
      .collect();
    let second_chunks: Vec<ExtensionTarget<D>> = (0..16)
      .map(|i| vars.local_wires[Self::wire_second_chunk_val(i)])
      .collect();
    ...
    let le = most_significant_diff_bits[Self::chunk_bits()];
    let one = builder.one_extension();
    let le_one = builder.sub_extension(one, le);
    let second = builder.sub_extension(result, second_input);
    let first = builder.sub_extension(result, first_input);
    constraints.push(builder.mul_extension(le_one, second));
    constraints.push(builder.mul_extension(le, first));
    // let result_bool = vars.local_wires[Self::wire_result_bool()];
    // constraints
  ...
  for ComparisonGate<F, D>
{
  fn eval_unfiltered_base_packed<P: PackedField<Scalar = F>>(
    ...
    // Get chunks and assert that they match
    let first_chunks: Vec<_> = (0..16)
      .map(|i| vars.local_wires[Self::wire_first_chunk_val(i)])
      .collect();
    let second_chunks: Vec<_> = (0..16)
      .map(|i| vars.local_wires[Self::wire_second_chunk_val(i)])
      .collect();
    ...
    let result = vars.local_wires[Self::wire_result()];
    let le = most_significant_diff_bits[Self::chunk_bits()];
    yield_constr.one((P::ONES - le) * (result - second_input));
    yield_constr.one(le * (result - first_input));
  }
}

#[derive(Clone, Debug, Default)]
pub(crate) struct ComparisonGenerator<F: RichField + Extendable<D>, const D: usize> {
  row: usize,
  num_bits: usize,
  //   num_chunks: usize,
  _phantom: PhantomData<F>,
  ...
  for ComparisonGenerator<F, D>
{
  ...
  fn dependencies(&self) -> Vec<Target> {
    vec![
      ComparisonGate::<F, D>::wire_first_input(),
      ComparisonGate::<F, D>::wire_second_input(),
    ]
    .iter()
  ...
  fn run_once(&self, witness: &PartitionWitness<F>, out_buffer: &mut GeneratedValues<F>) {
    ...
    let first_input = get_local_wire(ComparisonGate::<F, D>::wire_first_input());
    let second_input = get_local_wire(ComparisonGate::<F, D>::wire_second_input());

    let first_input_u64 = first_input.to_canonical_u64();
    let second_input_u64 = second_input.to_canonical_u64();

    let result = if first_input_u64 <= second_input_u64 {
      F::from_canonical_u64(first_input_u64)
    } else {
      F::from_canonical_u64(second_input_u64)
    };

    let chunk_size = 1 << ComparisonGate::<F, D>::chunk_bits();
    let first_input_chunks: Vec<F> = (0..16)
      ...
      .collect();
    let second_input_chunks: Vec<F> = (0..16)
    ...
    let two_n = F::from_canonical_usize(1 << ComparisonGate::<F, D>::chunk_bits());
    let two_n_plus_msd = (two_n + most_significant_diff).to_canonical_u64();

    let msd_bits_u64: Vec<u64> = (0..ComparisonGate::<F, D>::chunk_bits() + 1)
    ...
    out_buffer.set_wire(
      local_wire(ComparisonGate::<F, D>::wire_result()),
      result,
    );
    out_buffer.set_wire(
      local_wire(ComparisonGate::<F, D>::wire_most_significant_diff()),
      most_significant_diff,
    );
    for i in 0..16 {
      out_buffer.set_wire(
        local_wire(ComparisonGate::<F, D>::wire_first_chunk_val(i)),
        first_input_chunks[i],
      );
      out_buffer.set_wire(
        local_wire(ComparisonGate::<F, D>::wire_second_chunk_val(i)),
        second_input_chunks[i],
      );
      out_buffer.set_wire(
        local_wire(ComparisonGate::<F, D>::wire_equality_dummy(i)),
        equality_dummies[i],
      );
      out_buffer.set_wire(
        local_wire(ComparisonGate::<F, D>::wire_chunks_equal(i)),
        chunks_equal[i],
      );
      out_buffer.set_wire(
        local_wire(ComparisonGate::<F, D>::wire_intermediate_value(i)),
        intermediate_values[i],
      );
    }
    for i in 0..ComparisonGate::<F, D>::chunk_bits() + 1 {
      out_buffer.set_wire(
        local_wire(ComparisonGate::<F, D>::wire_most_significant_diff_bit(i)),
        msd_bits[i],
      );
    }
  }
