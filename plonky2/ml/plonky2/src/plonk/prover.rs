...
>(
    ...
    ) in prover_data.lookup_rows.iter().enumerate()
    {
        ...
        let table_value_to_idx: HashMap<u32, usize> = common_data.luts[lut_index]
            .iter()
            .enumerate()
        ...
        for (inp_target, _) in prover_data.lut_to_lookups[lut_index].iter() {
            ...
            let idx = table_value_to_idx
                .get(&u32::try_from(inp_value.to_canonical_u64()).unwrap())
                .unwrap();
        ...
        for slot in (num_entries - remaining_slots)..num_entries {
            ...
            pw.set_target(inp_target, F::from_canonical_u32(first_inp_value));
            pw.set_target(out_target, F::from_canonical_u32(first_out_value));
