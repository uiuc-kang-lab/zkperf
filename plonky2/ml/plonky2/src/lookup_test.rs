...
fn test_one_lookup() -> anyhow::Result<()> {
    ...
    assert!(
        proof.public_inputs[2] == F::from_canonical_u32(out_a),
        "First lookup, at index {} in the Tip5 table gives an incorrect output.",
        proof.public_inputs[0]
    );
    assert!(
        proof.public_inputs[3] == F::from_canonical_u32(out_b),
...
pub fn test_two_luts() -> anyhow::Result<()> {
    ...
    assert!(
        proof.public_inputs[3] == F::from_canonical_u32(first_out),
        "First lookup, at index {} in the Tip5 table gives an incorrect output.",
        proof.public_inputs[0]
    );
    assert!(
        proof.public_inputs[4] == F::from_canonical_u32(second_out),
        "Second lookup, at index {} in the Tip5 table gives an incorrect output.",
        proof.public_inputs[1]
    );
    assert!(
        proof.public_inputs[2] == F::from_canonical_u32(s),
        "Sum between the first two LUT outputs is incorrect."
    );
    assert!(
        proof.public_inputs[5] == F::from_canonical_u32(final_out),
...
pub fn test_different_inputs() -> anyhow::Result<()> {
    ...
    let tab: Vec<u32> = SMALLER_TABLE.to_vec();
    ...
    pw.set_target(initial_a, F::from_canonical_u32(look_val_a));
    pw.set_target(initial_b, F::from_canonical_u32(look_val_b));
    ...
    assert!(
        proof.public_inputs[3] == F::from_canonical_u32(out_a),
        "First lookup, at index {} in the smaller LUT gives an incorrect output.",
        proof.public_inputs[0]
    );
    assert!(
        proof.public_inputs[4] == F::from_canonical_u32(out_b),
    ...
    assert!(
        proof.public_inputs[5] == F::from_canonical_u32(out_final),
        "Output of the second LUT at index {} is incorrect.",
        s
    );
...
fn test_big_lut() -> anyhow::Result<()> {
    ...
    let inputs: [u32; LUT_SIZE] = core::array::from_fn(|i| i as u32);
    let lut_fn = |inp: u32| inp / 10;
    ...
    assert_eq!(
        proof.public_inputs[0],
        F::from_canonical_u32(lut_fn(look_val_a))
    );
    assert_eq!(
        proof.public_inputs[1],
        F::from_canonical_u32(lut_fn(look_val_b))
    );
...
fn test_many_lookups_on_big_lut() -> anyhow::Result<()> {
    ...
    assert_eq!(
        proof.public_inputs[0],
        F::from_canonical_u32(lut_fn(look_val_a) + lut_fn(look_val_b))
    );
