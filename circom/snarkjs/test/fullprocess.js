...
describe("Full process", function ()  {
    ...
    it ("R1CS info", async () => {
        await snarkjs.r1cs.info(path.join("test", "circuit", "circuit.r1cs"), logger);
    });
    ...
    it ("witness calculate", async () => {
        await snarkjs.wtns.calculate({a: 11, b:2}, path.join("test", "circuit", "circuit.wasm"), wtns, {"logger": logger});
    });
    ...
    it ("groth16 proof", async () => {
        const res = await snarkjs.groth16.prove(zkey_final, wtns, logger);
        proof = res.proof;
