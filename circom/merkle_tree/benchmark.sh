...
start=`date +%s%N`
/usr/bin/time -v npx snarkjs groth16 prove "$BUILD_DIR"/"$CIRCUIT_NAME".zkey "$BUILD_DIR"/witness.wtns "$BUILD_DIR"/proof.json "$BUILD_DIR"/public.json
end=`date +%s%N`
echo "Proof Size: $(stat -c %s "$BUILD_DIR"/proof.json)"
echo "Public Size: $(stat -c %s "$BUILD_DIR"/public.json)"
