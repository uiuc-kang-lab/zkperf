...
node ../snarkjs/cli.js -v groth16 prove "$BUILD_DIR"/"$CIRCUIT_NAME".zkey "$BUILD_DIR"/witness.wtns "$BUILD_DIR"/proof.json "$BUILD_DIR"/public.json > mnist_prover_breakdown.txt
node ../snarkjs/cli.js -v groth16 verify "$BUILD_DIR"/vkey.json "$BUILD_DIR"/public.json "$BUILD_DIR"/proof.json > mnist_verifier_breakdown.txt
python ../breakdown/breakdown.py mnist mnist_prover_breakdown.txt ../breakdown/mnist_prover_breakdown.json
python ../breakdown/breakdown.py mnist mnist_verifier_breakdown.txt ../breakdown/mnist_verifier_breakdown.json
