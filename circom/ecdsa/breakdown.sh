...
node ../snarkjs/cli.js -v groth16 prove "$BUILD_DIR"/"$CIRCUIT_NAME".zkey "$BUILD_DIR"/witness.wtns "$BUILD_DIR"/proof.json "$BUILD_DIR"/public.json > ecdsa_prover_breakdown.txt
node ../snarkjs/cli.js -v groth16 verify "$BUILD_DIR"/vkey.json "$BUILD_DIR"/public.json "$BUILD_DIR"/proof.json > ecdsa_verifier_breakdown.txt
python ../breakdown/breakdown.py ecdsa ecdsa_prover_breakdown.txt ../breakdown/ecdsa_prover_breakdown.json
python ../breakdown/breakdown.py ecdsa ecdsa_verifier_breakdown.txt ../breakdown/ecdsa_verifier_breakdown.json
