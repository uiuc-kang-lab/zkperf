...
use halo2_proofs::halo2curves::ff::{PrimeField, FromUniformBytes};
use halo2_proofs::{
    circuit::Value,
    plonk::{
...
use plonkish_backend::backend::{PlonkishCircuitInfo, WitnessEncoding, PlonkishCircuit};
...
#[derive(Clone, Debug)]
pub struct HyperModel<F: PrimeField, C: Circuit<F>> {
    pub k: u32,
    ...
    pub advice_idx_in_phase: Vec<usize>,
    pub challenge_idx: Vec<usize>,
    pub row_mapping: Vec<usize>,
}

impl<F: PrimeField, C: Circuit<F>> HyperModel<F, C> {
    pub fn new<E: WitnessEncoding>(k: usize, circuit: C) -> Self {
        ...
        let instances = vec![get_public_values()];
    ...
    pub fn instances(&self) -> Vec<Vec<F>> {
        vec![get_public_values()]
    }
}

impl<F: PrimeField + Ord + FromUniformBytes<64>, C: Circuit<F>> PlonkishCircuit<F> for HyperModel<F, C>{
    fn circuit_info_without_preprocess(&self) -> Result<PlonkishCircuitInfo<F>, plonkish_backend::Error> {
        let Self {
            k,
            instances,
            cs,
    ...
    fn circuit_info(&self) -> Result<PlonkishCircuitInfo<F>, plonkish_backend::Error> {
        let Self {
            k,
            instances,
            cs,
        ...
        .map_err(|err| plonkish_backend::Error::InvalidSnark(format!("Synthesize failure: {err:?}")))?;
    ...
    fn synthesize(&self, phase: usize, challenges: &[F]) -> Result<Vec<Vec<F>>, plonkish_backend::Error> {
