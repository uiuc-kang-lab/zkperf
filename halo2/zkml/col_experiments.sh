...
do
    ...
    ./target/release/time_circuit examples/dlrm/dlrm_model.msgpack examples/dlrm/dlrm_input.msgpack kzg > $OUTPUT_DIR/dlrm_test_$i_$K.txt 2>error.txt
    if [ $(cat error.txt | grep -c "NotEnoughRowsAvailable") -ne 0 ]; then
        rm $OUTPUT_DIR/dlrm_test_$i_$K.txt
        rm error.txt
        K=$(expr $K + 1)
        python3 python/converter.py --model examples/dlrm/dlrm_mlp_checked_float32.tflite --model_output examples/dlrm/dlrm_model.msgpack --config python/dlrm_config.msgpack --scale_factor 512 --num_cols $i --num_witness_cols $(expr $i / 2) --k $K
        ./target/release/time_circuit examples/dlrm/dlrm_model.msgpack examples/dlrm/dlrm_input.msgpack kzg > $OUTPUT_DIR/dlrm_test_$i_$K.txt 2>error.txt
